...
def search_optimal_parameters_for_gradient_boost_min_value_prediction(train_df:pd.DataFrame) -> None:
...
def predict_min_value_with_gradient_boost_regressor(df:pd.DataFrame, test_df:pd.DataFrame, validation_df:pd.DataFrame, base_dir:str):
    ...
    with open(base_dir + '/xgboost_regressor_predict_min_values-2025-03-27.pkl', 'wb') as file:
    ...
    with open(base_dir + '/xgboost_regressor_predict_min_values_after_test_training-2025-03-27.pkl', 'wb') as file:
        pickle.dump(logreg_model, file)
