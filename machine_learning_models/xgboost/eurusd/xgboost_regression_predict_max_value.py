...
def search_optimal_parameters_for_gradient_boost_max_value_prediction(train_df:pd.DataFrame) -> None:
    ...
    logreg_model = GradientBoostingRegressor(random_state=1, verbose=1)
...
def predict_max_value_with_gradient_boost_regressor(df:pd.DataFrame, test_df:pd.DataFrame, validation_df:pd.DataFrame, base_dir:str):
    ...
    logreg_model = GradientBoostingRegressor(random_state=1, n_estimators=100, max_depth=5, min_samples_split=10, min_samples_leaf=3, max_features='sqrt', verbose=2)

    logreg_model.fit(X_train, y_train)

    with open(base_dir + '/xgboost_regressor_predict_max_values-2025-03-27.pkl', 'wb') as file:
    ...
    with open(base_dir + '/xgboost_regressor_predict_max_values_after_test_training-2025-03-27.pkl', 'wb') as file:
        pickle.dump(logreg_model, file)
