...
def predict_max_value_with_random_forest_regressor(df:pd.DataFrame, test_df:pd.DataFrame, validation_df:pd.DataFrame, base_dir:str):
    ...
    logreg_model = RandomForestRegressor(random_state=1, criterion='absolute_error', n_estimators=50, max_depth=5, min_samples_split=10, min_samples_leaf=3, max_features='sqrt', n_jobs=10, max_samples=0.8, verbose=2)

    logreg_model.fit(X_train, y_train)

    with open(base_dir + '/random_forest_regressor_predict_max_values-2025-03-25.pkl', 'wb') as file:
