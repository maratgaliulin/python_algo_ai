...
def predict_min_value_with_random_forest_regressor(train_df:pd.DataFrame, test_df:pd.DataFrame, validation_df:pd.DataFrame, base_dir:str):
    X_train = train_df.drop(columns=['y_60min_max', 'y_60min_min', 'trend', 'y_trend_downtrend', 'y_trend_uptrend', 'y_trend_trend undefined'])
    ...
    with open(base_dir + '/random_forest_regressor_predict_min_values_after_test_training.pkl', 'wb') as file:
        pickle.dump(logreg_model, file)
